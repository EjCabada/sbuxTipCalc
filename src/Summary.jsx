  ...
  return (
    <div className="summary-container">
      ...
      <table className="summary-table">
        ...
        <tbody>
          {employeeData.map((emp, index) => {
            // Explicit 3-row alternating group shading
            ...
            return (
              <tr key={emp.employeeNumber || index} className={rowClass}>
                ...
                <td>{Number(emp.hoursWorked).toFixed(2)} hrs</td>
                <td>
                  <button className="edit-partner-btn" onClick={() => onEditPartner(index)}>
                    Edit
                  </button>
                </td>
              </tr>
      ...
      <div className="statsBox" style={{ marginTop: '1.5em' }}>
        <p>
          <strong>Total Calculated Hours:</strong> {totalHoursCalculated.toFixed(2)} hrs
        </p>
      </div>

      <div className="summary-actions" style={{ marginTop: '1em' }}>
        <button onClick={onBackToInput} style={{ backgroundColor: '#6c757d' }}>
          Add More Partners
        </button>
        <button onClick={onConfirm} style={{ backgroundColor: 'var(--sbuxbrightgreen)' }}>
          Confirm & Continue
        </button>
      </div>
    </div>
